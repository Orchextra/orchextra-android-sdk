...
fun OxDevice.toApiOxDevice(): ApiOxDevice = with(this) {
  return ApiOxDevice(
      handset = if (handset.isNotEmpty()) {
        handset
      } else {
        null
      },
      language = if (language.isNotEmpty()) {
        language
      } else {
        null
      },
      osVersion = if (osVersion.isNotEmpty()) {
        osVersion
      } else {
        null
      },
      timeZone = if (timeZone.isNotEmpty()) {
        timeZone
      } else {
        null
      },
      instanceId = if (instanceId.isNotEmpty()) {
        instanceId
      } else {
        null
      },
      secureId = if (secureId.isNotEmpty()) {
        secureId
      } else {
        null
      },
      serialNumber = if (serialNumber.isNotEmpty()) {
        serialNumber
      } else {
        null
      },
      bluetoothMacAddress = if (bluetoothMacAddress.isNotEmpty()) {
        bluetoothMacAddress
      } else {
        null
      },
      wifiMacAddress = if (wifiMacAddress.isNotEmpty()) {
        wifiMacAddress
      } else {
        null
      },
      ...
      businessUnits = if (businessUnits.isNotEmpty()) {
        businessUnits
      } else {
        null
      }
