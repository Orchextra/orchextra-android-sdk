...
object Orchextra : OrchextraErrorListener {
    ...
    @JvmOverloads
    fun init(
        context: Application, apiKey: String, apiSecret: String,
    ...
    private fun getConfiguration(
        context: Context,
        apiKey: String,
        configurationSuccess: () -> Unit
    ) {
        val getConfiguration = GetConfiguration.create(
            NetworkDataSource.create(context),
            dbDataSource
        )
    ...
    fun setNotificationActivityClass(notificationActivityClass: Class<*>) {
        notificationActivityClass.canonicalName?.let {
            dbDataSource.saveNotificationActivityName(it)
        }
    }
