...
public class ValidationError implements InteractorError {
  ...
  public String getValidationErrorDescriptionString() {
    String validationError =
        "Error code: " + businessError.getCode() + " --> " + businessError.getMessage() + "\n";

    if (businessError.getExtraErrorInfo().size() > 0) {
      for (DomainErrorData ded : businessError.getExtraErrorInfo()) {
        validationError =
            validationError + ded.getField() + ": " + ded.getErrorDescription() + "\n";
      }
    }
