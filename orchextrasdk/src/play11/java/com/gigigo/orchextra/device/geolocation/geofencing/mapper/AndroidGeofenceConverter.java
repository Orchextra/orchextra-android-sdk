...
public class AndroidGeofenceConverter {
  ...
  public GeofencingRequest convertGeofencesToGeofencingRequest(
    ...
    while (i < orchextraGeofenceList.size() && i < ConstantsAndroidGeofence.MAX_NUM_GEOFENCES) {
      ...
      System.out.println(
          "Geofence Register:" + orchextraGeofence.getGeofenceId() + " Radius: " + orchextraGeofence
              .getRadius());
      Geofence geofence = new Geofence.Builder().setRequestId(orchextraGeofence.getCode())
          .setCircularRegion(orchextraGeofence.getPoint().getLat(),       orchextraGeofence.getPoint().getLng(),  GeofenceUtils.getRadius(orchextraGeofence.getRadius()))
          .setNotificationResponsiveness(0)//new
          .setExpirationDuration(Geofence.NEVER_EXPIRE)
          .setLoiteringDelay(GeofenceUtils.getStayTimeDelayMs(
              orchextraGeofence.getStayTime()))  // Required when we use the transition type of GEOFENCE_TRANSITION_DWELL
          .setTransitionTypes(GeofenceUtils.getTransitionTypes(orchextraGeofence.isNotifyOnEntry(),
              orchextraGeofence.isNotifyOnExit()))
          .build();
    ...
    GeofencingRequest geofencingRequest =
        new GeofencingRequest.Builder().addGeofences(androidGeofenceList)
            .setInitialTrigger(
